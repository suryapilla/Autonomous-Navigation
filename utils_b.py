...
class robot_env:
    ...
    def robot_state(self):
        ...
        self.door_1_lock_stat = not self.info["door_open"][0]
        
        self.door_2_lock_stat = not self.info["door_open"][1]
        
        self.key_pos = sum(self.info["key_pos"])
        
        self.goal_pos = sum(self.info["goal_pos"])
        ...
        return self.x_pos, self.y_pos, self.orient, self.key_stat, self.door_1_lock_stat, self.door_2_lock_stat, self.key_pos, self.goal_pos
    ...
    def robot_map(self):
        ...
        self.x_door_1 = self.info["door_pos"][0][0]
        self.y_door_1 = self.info["door_pos"][0][1]
        self.x_door_2 = self.info["door_pos"][1][0]
        self.y_door_2 = self.info["door_pos"][1][1]
        ...
        # return self.x_goal, self.y_goal, self.x_start, self.y_start, self.x_key, self.y_key, self.x_door_1, self.y_door_1, self.x_door_2, self.y_door_2, self.xy_wall
    
    
        return self.x_start, self.y_start, self.x_door_1, self.y_door_1, self.x_door_2, self.y_door_2, self.xy_wall
...
class dp(robot_env):
    def __init__(self,env,info):
        ...
        self.T_horiz = self.env.height*self.env.width*4*2*2*2*3*3
        self.len_state_space = self.env.height*self.env.width*4*2*2*2*3*3
        ...
        self.x_goal = self.info["goal_pos"][0]
        self.x_goal = self.info["goal_pos"][1]
        
        self.key_loc_dict = {2:[1,1],5:[2,3],7:[1,6]}
        self.goal_loc_dict = {6:[5,1],9:[6,3],11:[5,6]}
    ...
    def state_space(self):
        ...
        door_stat_1 = np.array([0, 1])
        door_stat_2 = np.array([0, 1])
        
        key_loc = np.array([2,5,7])
        
        goal_loc = np.array([6,9,11])
        
        state_space_N = np.array(list(product(x, y, orient, key_stat, door_stat_1,door_stat_2,key_loc,goal_loc)))
    ...
    def motion_model_robot(self,state,action):
        ...
        # state = x-0, y-1, orient-2, key_stat-3, door_locked_1-4,door_locked_2-5,key_loc-6,goal_loc-7
        self.prev_state = state.copy()
        self.curr_state = state.copy()
        self.key = self.key_loc_dict[state[6]]
        self.x_key = self.key[0]
        self.y_key = self.key[1]
        
        if action == "MF" and state[2] == 0:
            if (state[0] + 1 == self.x_key and state[1] == self.y_key) and state[3]==0:
                self.curr_state[0] = state[0]
            elif (state[0] + 1 == self.x_door_1 and state[1] == self.y_door_1) and state[4]==1:
                self.curr_state[0] = state[0]
            elif (state[0] + 1 == self.x_door_2 and state[1] == self.y_door_2) and state[5]==1:
                self.curr_state[0] = state[0]
            ...
            elif (state[0] == self.x_door_1 and state[1] + 1 == self.y_door_1) and state[4]==1:
                self.curr_state[1] = state[1]
            elif (state[0] == self.x_door_2 and state[1] + 1 == self.y_door_2) and state[5]==1:
                self.curr_state[1] = state[1]
            ...
            if state[2] == 0: #Right
                if self.x_key == min(state[0] + 1, self.N -1) and self.y_key == state[1] and (state[4] or state[5]):
                    self.curr_state[3] = 1

            elif state[2] == 1: #Down
                if self.x_key == state[0] and self.y_key == min(state[1] + 1, self.N -1) and (state[4] or state[5]):
                    self.curr_state[3] = 1
            elif state[2] == 2: #Left
                if self.x_key == max(state[0]-1,0) and self.y_key == state[1] and (state[4] or state[5]):
                    self.curr_state[3] = 1
            elif state[2] == 3: #UP
                if self.x_key == state[0] and self.y_key == max(state[1]-1,0) and (state[4] or state[5]):
                    self.curr_state[3] = 1
            
        elif action == "UD":
            
            if state[2] == 0 and state[3]==1: 
                if self.x_door_1 == min(state[0] + 1,self.N -1) and self.y_door_1 == state[1]:
                    self.curr_state[4] = 0
            elif state[2] == 1 and state[3]==1: 
                if self.x_door_1 == state[0] and self.y_door_1 == min(state[1] + 1,self.N -1):
                    self.curr_state[4] = 0
            elif state[2] == 2 and state[3]==1:
                if self.x_door_1 == max(state[0]-1,0) and self.y_door_1 == state[1]:
                    self.curr_state[4] = 0
            elif state[2] == 3 and state[3]==1: 
                if self.x_door_1 == state[0] and self.y_door_1 == max(state[1]-1,0):
                    self.curr_state[4] = 0
            
            elif state[2] == 0 and state[3]==1: 
                if self.x_door_2 == min(state[0] + 1,self.N -1) and self.y_door_2 == state[1]:
                    self.curr_state[5] = 0
            elif state[2] == 1 and state[3]==1:
                if self.x_door_2 == state[0] and self.y_door_2 == min(state[1] + 1,self.N -1):
                    self.curr_state[5] = 0
            elif state[2] == 2 and state[3]==1:
                if self.x_door_2 == max(state[0]-1,0) and self.y_door_2 == state[1]:
                    self.curr_state[5] = 0
            elif state[2] == 3 and state[3]==1:
                if self.x_door_2 == state[0] and self.y_door_2 == max(state[1]-1,0):
                    self.curr_state[5] = 0
        
        if action=="UD" or action=="PK":
            self.prev_state[3] = self.curr_state[3]
            self.prev_state[4] = self.curr_state[4]
            self.prev_state[5] = self.curr_state[5]
    ...
    def stage_cost(self,state,action):
        ...
        if action == "MF":
            ...
            if self.isWall([self.nxt_state[0],self.nxt_state[1]]):
                ...
                elif self.nxt_state[0] == self.x_door_1 and self.nxt_state[1] == self.y_door_1 and state[4] == 1:
                    return np.inf
                
                elif self.nxt_state[0] == self.x_door_2 and self.nxt_state[1] == self.y_door_2 and state[5] == 1:
                    return np.inf
            ...
            if self.nxt_state[0] == self.x_key and self.nxt_state[1] == self.y_key and state[3]==0 and (state[4] or state[5]):
                return 1
            else:
                return np.inf
            ...
            if self.nxt_state[0] == self.x_door_1 and self.nxt_state[1] == self.y_door_1 and state[3]==1 and state[4] == 1:
                return 1
            if self.nxt_state[0] == self.x_door_2 and self.nxt_state[1] == self.y_door_2 and state[3]==1 and state[5] == 1:
                return 1
            else:
                return np.inf
    ...
    def algo_dp(self):
        ...
        orient, key_stat, door1_stat, door2_stat, key_pos, goal_pos = np.array(list(product(range(4), range(2), range(2), range(2), np.array([2,5,7]), np.array([6,9,11])))).T
        # self.x_goal = self.goal_loc_dict[goal_pos]
        # self.y_goal = self.goal_loc_dict[goal_pos]
        for i in [6,9,11]:
            
            self.x_goal = self.goal_loc_dict[i][0]
            self.y_goal = self.goal_loc_dict[i][1]
            
            goal_state = np.column_stack([self.x_goal*np.ones(orient.shape), self.y_goal*np.ones(orient.shape), orient, key_stat, door1_stat, door2_stat, key_pos, i*np.ones(orient.shape)])
            goal_idx = np.where((self.state_space_N == goal_state[:, None]).all(-1))[1]
            self.val_t[goal_idx,self.T_horiz - 1] = -1
            self.Q_t[goal_idx] = -1
        
        for i in trange(self.T_horiz-2,-1,-1):
            for state_idx in range(self.state_space_N.shape[0]):
                x_t = self.state_space_N[state_idx].copy()
                
                if self.state_space_N[state_idx][0] == self.goal_loc_dict[x_t[7]][0] and self.state_space_N[state_idx][1] == self.goal_loc_dict[x_t[7]][1]:
             
                    continue 
                coords = np.array([self.state_space_N[state_idx][0],self.state_space_N[state_idx][1]])
  
                if self.isWall([coords[0],coords[1]]):
                    continue
                
                for action in range(5):  

                    l_t = self.stage_cost(self.state_space_N[state_idx],actions[action])

                    x_t_1 = self.motion_model_robot(x_t,actions[action])
                    
                    g_index = np.where((self.state_space_N == x_t_1).all(axis=1))[0][0]
    
                    self.Q_t[state_idx,action] = l_t + self.val_t[g_index,i+1]
...
def load_random_env(env_folder):
    ...
    # env_path = os.path.join(env_folder,"DoorKey-8x8-28.env")
...
def load_random_env_all(env_path):
    """
    Load a random DoorKey environment
    ---------------------------------------------
    Returns:
